...
from MTCNN.invalid_image import InvalidImage
...
class StageStatus(object):
    ...
    def __init__(self, pad_result:tuple = None, width=0, height=0):
        self.width = width
        self.height = height
        self.dx = self.edy = self.dx = self.edx = self.y = self.ey = self.x = self.ex = self.tmpw = self.tmph = []
...
class MTCNN(object):
    ...
    """
    Allows to perfrom MTCNN 
        a) Detection of face and crop face landmark.
        b) Detection of face and retangel face.
    """
    ...
    def __init__(self, weights_file: str = None, min_face_size: int = 20, steps_threshold: list = None,
        ...
        self._pnet, self._rnet, self._onet = NetworkFactory().build_nets_from_file(weights_file)
    ...
    def detect_faces(self, img):
        ...
        for bounding_box, keypoints in zip(total_boxes, points.T):
            bounding_boxes.append(
                [max(0, int(bounding_box[0])), max(0, int(bounding_box[1])),
